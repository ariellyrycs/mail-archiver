<template>
    <div class="date-range-picker-container">
        <a href="#" class="search-box-container" @click="openCalanedar">
            <div class="picker-input">
                <img src="../files/icon_calender.svg" />
                <span class="text">{{headStartingDateFormat}} - {{headEndingDateFormat}}</span>
            </div>
            <div class="btn-search">
                <img src="../files/icon_search.svg" />
            </div>
        </a>
        <div v-bind:class="['calendar-container', showCalendar ? '': 'hide']" @blur="openCalanedar">
            <div class="controls">
                <a href="#" @click="goPreviousMonth"><i class="left"></i></a>
                <span>{{monthDesciption}} {{year}}</span>
                <a href="#" @click="goNextMonth"><i class="right"></i></a>
            </div>
            <div class="weekdays">
                <span v-bind:key="weekday" v-for="weekday in weekdays">{{weekday}}</span>
            </div>
            <div class="days">
                <!-- a calendar contains 5 rows -->
                <ul v-for="(index, row) in days" v-bind:key="index">
                    <li v-for="(o, item) in row"
                        v-bind:key="o"
